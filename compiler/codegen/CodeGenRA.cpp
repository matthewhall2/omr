...
void OMR::CodeGenerator::checkForLiveRegisters(TR_LiveRegisters *liveRegisters)
{
    ...
    if (liveRegisters && liveRegisters->getFirstLiveRegister()) {
        ...
        for (TR_LiveRegisterInfo *p = liveRegisters->getFirstLiveRegister(); p; p = p->getNext()) {
            if (trace) {
                if (p->getNode())
                    log->printf("Virtual register %s (for node [%s], ref count=%d) is live at end of method\n",
                        debug->getName(p->getRegister()), debug->getName(p->getNode()),
                        p->getNode()->getReferenceCount());
                else
                    log->printf("Virtual register %s %p is live at end of method with node count %d\n",
...
void OMR::CodeGenerator::estimateRegisterPressure(TR::Node *node, int32_t &registerPressure,
    int32_t &maxRegisterPressure, int32_t maxRegisters, TR_BitVector *valuesInGlobalRegs, bool isCold,
    vcount_t visitCount, TR::SymbolReference *symRef, bool &symRefIsLive, bool checkForIMuls, bool &vmThreadUsed)
{
    if (node->getVisitCount() == visitCount) {
        node->setLocalIndex(node->getLocalIndex() - 1);
        if (node->getLocalIndex() == 0) {
            if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() && valuesInGlobalRegs
                ...
                if (!node->getOpCode().isLoadConst()) {
                    registerPressure--;
                    if ((node->getType().isInt64()) && self()->comp()->target().is32Bit())
                        registerPressure--;
                }
    ...
    for (childCount = node->getNumChildren() - 1; childCount >= 0; childCount--)
        self()->estimateRegisterPressure(node->getChild(childCount), registerPressure, maxRegisterPressure,
            maxRegisters, valuesInGlobalRegs, isCold, visitCount, symRef, symRefIsLive, checkForIMuls, vmThreadUsed);

    bool highRegisterPressureOpCode = false;
    if (node->getOpCode().isResolveCheck() || (node->getOpCode().isCall()) || node->getOpCodeValue() == TR::New
            || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray
            || node->getOpCodeValue() == TR::multianewarray || node->getOpCodeValue() == TR::monent
            || node->getOpCodeValue() == TR::monexit || node->getOpCodeValue() == TR::checkcast
            || node->getOpCodeValue() == TR::
            instanceof || node->getOpCodeValue() == TR::arraycopy)
        highRegisterPressureOpCode = true;

    if (highRegisterPressureOpCode || node->getOpCodeValue() == TR::asynccheck)
        vmThreadUsed = true;
    ...
    if (node->getLocalIndex() > 0) {
        if (node->getOpCode().isLoadVar() && node->getSymbol()->isAutoOrParm() && valuesInGlobalRegs
            ...
            if (!node->getOpCode().isLoadConst()) {
                ...
                if (dtype == TR::Float || dtype == TR::Double)
                    gprCandidate = false;
            ...
            if (!symRefIsLive) {
                ...
                if (highRegisterPressureOpCode
                    || ((node->getType().isInt64()) && self()->comp()->target().is32Bit()
                        && (node->getOpCode().isMul() || node->getOpCode().isDiv() || node->getOpCode().isRem()
                            || node->getOpCode().isLeftShift() || node->getOpCode().isRightShift()
                            || node->getOpCode().isBooleanCompare()))) {
                    if (!isCold) {
                        if (registerPressure >= (maxRegisters - NUM_REGS_USED_BY_COMPLEX_OPCODES
                                - 1)) // Only allow if register pressure is known to be very low
                        {
                            maxRegisterPressure = maxRegisters;
                        }
                    }
                } else if (checkForIMuls && (node->getOpCode().isMul() || node->getOpCode().isDiv())) {
                    if (!isCold) {
                        maxRegisterPressure = maxRegisters;
                    }
                } else {
...
int32_t OMR::CodeGenerator::estimateRegisterPressure(TR::Block *block, vcount_t visitCount, int32_t maxStaticFrequency,
    int32_t maxFrequency, bool &vmThreadUsed, int32_t numGlobalRegs, TR_BitVector *valuesInGlobalRegs,
    TR::SymbolReference *symRef, bool checkForIMuls)
{
    ...
    //
    if (block->isCold() || ((maxFrequency > 0) && (block->getFrequency() * 100 / maxFrequency < 20)))
        isCold = true;
    ...
    if (!isCold) {
        ...
        if ((maxStaticFrequency > 0) && (blockWeight * 100 / maxStaticFrequency < 20))
            isCold = true;
    }
    ...
    while (treeTop) {
        ...
        self()->estimateRegisterPressure(node, currRegisterPressure, maxRegisterPressure,
            self()->getMaximumNumbersOfAssignableGPRs(), valuesInGlobalRegs, isCold, visitCount, symRef, symRefIsLive,
            checkForIMuls, vmThreadUsed);

        if (vmThreadUsed && (maxRegisterPressure >= self()->getMaximumNumbersOfAssignableGPRs())) {
            // dumpOptDetails("For global register candidate %d reg pressure reached limit %d at block_%d isCold %d
            // maxFreq %d\n", rc->getSymbolReference()->getReferenceNumber(), maxRegisterPressure, liveBlockNum, isCold,
            // maxFrequency);
            break;
        }
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (node->getBlock()->isCold()
                || ((maxFrequency > 0) && (node->getBlock()->getFrequency() * 100 / maxFrequency < 20)))
                isCold = true;
            else {
                TR_BlockStructure *blockStructure = block->getStructureOf();
                int32_t blockWeight = 1;
...
// need to be assigned.
//
TR_RegisterKinds OMR::CodeGenerator::prepareRegistersForAssignment()
{
...
TR_BackingStore *OMR::CodeGenerator::allocateInternalPointerSpill(TR::AutomaticSymbol *pinningArrayPointer)
{
    // Search for an existing free spill slot for this pinningArrayPointer
    //
    TR_BackingStore *spill = NULL;
    for (auto i = self()->getInternalPointerSpillFreeList().begin();
         i != self()->getInternalPointerSpillFreeList().end(); ++i) {
        if ((*i)->getSymbolReference()
                ->getSymbol()
                ->getAutoSymbol()
    ...
    if (!spill) {
        int32_t slot;
        TR::AutomaticSymbol *spillSymbol = TR::AutomaticSymbol::createInternalPointer(self()->trHeapMemory(),
            TR::Address, static_cast<uint32_t>(TR::Compiler->om.sizeofReferenceAddress()), self()->fe());
...
TR_BackingStore *OMR::CodeGenerator::allocateSpill(bool containsCollectedReference, int32_t *offset, bool reuse)
{
    return self()->allocateSpill((int32_t)(TR::Compiler->om.sizeofReferenceAddress()), containsCollectedReference,
        offset, reuse); // cast parameter explicitly
}

TR_BackingStore *OMR::CodeGenerator::allocateSpill(int32_t dataSize, bool containsCollectedReference, int32_t *offset,
    bool reuse)
{
    TR_ASSERT_FATAL(dataSize <= 64, "Spill size must be <= 64 bytes");
    TR_ASSERT_FATAL(!containsCollectedReference || (dataSize == TR::Compiler->om.sizeofReferenceAddress()),
        "assertion failure");
    ...
    //
    bool try8ByteSpills
        = (dataSize < 16) && ((TR::Compiler->om.sizeofReferenceAddress() == 8) || !containsCollectedReference);
    ...
    if (reuse) {
        ...
        if ((spill && comp->getOption(TR_TraceRA)
                && !performTransformation(comp, "O^O SPILL TEMPS: Reuse spill temp %s\n",
    ...
    if (spill) {
        ...
        spill = new (self()->trHeapMemory()) TR_BackingStore(comp->getSymRefTab(), spillSymbol, 0);
    ...
    if (dataSize <= 4 && spillSymbol->getSize() == 8) {
        ...
        //
        if (offset == NULL || spill->secondHalfIsOccupied()
...
void OMR::CodeGenerator::freeSpill(TR_BackingStore *spill, int32_t dataSize, int32_t offset)
{
    ...
    logprintf(trace, log, "\nfreeSpill(%s(%d%d), %d, %d, isLocked=%d)",
        self()->getDebug()->getName(spill->getSymbolReference()->getSymbol()), spill->firstHalfIsOccupied() ? 1 : 0,
        spill->secondHalfIsOccupied() ? 1 : 0, dataSize, offset, self()->isFreeSpillListLocked());
    ...
    if (spill->getSymbolReference()->getSymbol()->getAutoSymbol()->isInternalPointer()) {
        ...
        if (spill->isEmpty()) {
            ...
            // TODO: Once every caller can cope with nonzero offsets, we should add first-half-occupied symbols into
            // _spill4FreeList.
...
{
    logprints(self()->comp()->getOption(TR_TraceRA), self()->comp()->log(),
        "jettisonAllSpills: Clearing spill-temp freelists\n");
...
void OMR::CodeGenerator::findCommonedReferences(TR::Node *node, TR::TreeTop *treeTop)
{
    ...
    for (int32_t childCount = node->getNumChildren() - 1; childCount >= 0; childCount--) {
        ...
        if (child->getDataType() == TR::Address && !child->getOpCode().isLoadConst()
...
void OMR::CodeGenerator::processReference(TR::Node *reference, TR::Node *parent, TR::TreeTop *treeTop)
{
...
void OMR::CodeGenerator::spillLiveReferencesToTemps(TR::TreeTop *insertionTree,
    std::list<TR::SymbolReference *, TR::typed_allocator<TR::SymbolReference *, TR::Allocator> >::iterator
        firstAvailableSpillTemp)
{
    ...
    while (referenceIterator != _liveReferenceList.end()) {
        ...
        if (originalReference->getOpCode().hasSymbolReference() && originalReference->getSymbol()->isSpillTempAuto()) {
        ...
        while (nodeCursor) {
            for (int32_t childCount = 0; childCount < nodeCursor->getNumChildren(); ++childCount) {
        ...
        if (tempSymRef) {
            ...
            if (storeSpillTemp) {
                ...
                TR::TreeTop::create(self()->comp(), insertionTree,
                    TR::Node::createWithSymRef(TR::astore, 1, 1, newReference, tempSymRef));
            }
        ...
        //
        originalReference->setReferenceCount(
            originalReference->getReferenceCount() - (*referenceIterator)->getNumberOfParents());
...
void OMR::CodeGenerator::needSpillTemp(TR_LiveReference *cursor, TR::Node *parent, TR::TreeTop *treeTop)
{
    // If the reference is a simple load and the load is not killed before the
    // first use after this GC point, there is no need for a spill temp, the
    // original load can just be duplicated at the first use after this point.
...
bool OMR::CodeGenerator::allowGlobalRegisterAcrossBranch(TR::RegisterCandidate *, TR::Node *branchNode)
{
...
void OMR::CodeGenerator::initializeRegisterPressureSimulator()
{
...
bool OMR::CodeGenerator::prepareForGRA()
{
...
static bool blockIsMuchColderThanContainingLoop(TR::Block *block, TR::CodeGenerator *cg)
{
    ...
    bool trace = cg->traceSimulateTreeEvaluation();
    ...
    if (cg->comp()->getMethodHotness() <= warm) {
        ...
        if (block->getFrequency() < regSimBlockFreqCutoff) {
            logprintf(trace, log, "            Block %d is not hot enough for simulation (%d)\n", block->getNumber(),
                block->getFrequency());
            return true;
        }
    }
    ...
    if (result)
        logprintf(trace, log, "            Block %d is much colder than containing loop (%d << %d)\n",
...
{
    if (block->isCold())
        logprintf(cg->traceSimulateTreeEvaluation(), cg->comp()->log(), "            Block %d is cold\n",
            block->getNumber());
    return (block->isCold() || blockIsMuchColderThanContainingLoop(block, cg));
}

void OMR::CodeGenerator::TR_RegisterPressureState::updateRegisterPressure(TR::Symbol *symbol)
{
    ...
    if (symbol->getType().isAggregate()) {
        ...
        logprintf(comp->getOption(TR_TraceRegisterPressureDetails), comp->log(),
...
TR_GlobalRegisterNumber OMR::CodeGenerator::pickRegister(TR::RegisterCandidate *rc, TR::Block **allBlocks,
    ...
    TR_LinkHead<TR::RegisterCandidate> *candidatesAlreadyAssigned)
{
    ...
    static volatile uint8_t gprsWithheldFromPickRegister = 0, fprsWithheldFromPickRegister = 0,
                            vrfWithheldFromPickRegister = 0, gprsWithheldFromPickRegisterWhenWarm = 0;
    ...
    if (!comp->getOption(TR_DisableRegisterPressureSimulation)) {
        ...
        if (spilledRegisters) {
            if (traceSimulateTreeEvaluation) {
                ...
                log->prints("            vmThread register not enabled; rejected: ");
                self()->getDebug()->print(log, &regsToPrint);
                log->println();
            }
        ...
        TR_BitVector alreadyAssignedOnEntry(comp->getSymRefCount(), self()->trMemory(), stackAlloc, growable);
        TR_BitVector alreadyAssignedOnExit(comp->getSymRefCount(), self()->trMemory(), stackAlloc, growable);

        TR_BitVector blocksToVisit = rc->getBlocksLiveOnEntry();
        blocksToVisit |= rc->getBlocksLiveOnExit();

        if (traceSimulateTreeEvaluation) {
            log->prints("                Blocks to visit: ");
            self()->getDebug()->print(log, &blocksToVisit);
            log->println();
        }
        ...
        if (terseSimulateTreeEvaluation) {
            log->prints("            Ext blocks to visit: ");
            self()->getDebug()->print(log, &blocksToVisit);
            log->println();
        }
        ...
        while (blockIterator.hasMoreElements()) {
            ...
            if (!block->isExtensionOfPreviousBlock() && !blockIsIgnorablyCold(block, self())) {
                ...
                //
                static int32_t meniscus
                    = feGetEnv("TR_registerPressureMeniscus") ? atoi(feGetEnv("TR_registerPressureMeniscus")) : 0;
                ...
                TR_RegisterPressureState blockEntryState(NULL, false, alreadyAssignedOnEntry, alreadyAssignedOnExit,
                    candidatesAlreadyAssigned, self()->getNumberOfGlobalGPRs() + meniscus - gprDelta,
                    self()->getNumberOfGlobalFPRs() + meniscus - fprDelta,
                    self()->getNumberOfGlobalVRFs() + meniscus - vrfDelta, 0);
                for (TR::RegisterCandidate *candidate = candidatesAlreadyAssigned->getFirst(); candidate;
                ...
                if (cacheIsInconclusive) {
                    if (canAffordFullSimulation) {
                        TR_RegisterPressureState state(blockEntryState, rc,
                            rc->getBlocksLiveOnEntry().isSet(block->getNumber()), comp->incVisitCount());
                        ...
                        _blockAndCandidateRegisterPressureCacheTags[block->getNumber()]
                        ...
                        logprintf(traceSimulateTreeEvaluation, log, "            saved summary for block_%d\n",
                            block->getNumber());
                    } else {
                        // Can't afford a full simulation; assume GRA is no longer profitable.
                        //
                        logprints(terseSimulateTreeEvaluation, log,
                            "         } can't afford full simulation -- exiting\n");
                    ...
                    //
                    if (traceSimulateTreeEvaluation) {
                        log->printf("            using cache for block_%d -- g=%d, f=%d, v=%d", block->getNumber(),
                            cachedSummary->_gprPressure, cachedSummary->_fprPressure, cachedSummary->_vrfPressure);
                ...
                if (canExitEarly) {
                    ...
                    if (traceSimulateTreeEvaluation) {
                        log->prints("            rejected registers: ");
                        self()->getDebug()->print(log, &spilledRegs);
                        highWaterMark.dumpSpillMask(self());
                        log->prints("\n            remaining registers: ");
                        self()->getDebug()->print(log, &remainingRegisters);
                        log->println();
                    }
                }
            }
        ...
        while (blockIterator.hasMoreElements()) {
            ...
            if (block != comp->getFlowGraph()->getEnd() && !blockIsIgnorablyCold(block, self())) {
                ...
                //
                int32_t blockWeight = 1; // Note that we use a static loop-nesting-depth heuristic here for consistency
                                         // with the rest of GRA
                ...
                if (blockWeight > hottestCopiedRegisterWeight) {
                    logprintf(comp->getOption(TR_TraceRegisterPressureDetails), log,
                        "            scanning block_%d for copies\n", block->getNumber());
                    ...
                    for (TR::TreeTop *tt = block->getEntry(); tt != stopTree; tt = tt->getNextTreeTop()) {
                        ...
                        TR::RegisterCandidate *candidate
                            = self()->findCoalescenceForRegisterCopy(node, rc, &isUnpreferred);
                        ...
                        if (candidate) {
                            if (!isUnpreferred) {
                                ...
                                if (remainingRegisters.isSet(preferredRegister)) {
                                    ...
                                    logprintf(traceSimulateTreeEvaluation, log,
                                        "            found copy to/from #%d, register %d (%s), weight %d, at %s\n",
                                        candidate->getSymbolReference()->getReferenceNumber(), hottestCopiedRegister,
                                        self()->getDebug()->getGlobalRegisterName(hottestCopiedRegister),
                                        hottestCopiedRegisterWeight, self()->getDebug()->getName(node));
                                }
                            } else // is Unpreferred
                            {
                                logprintf(traceSimulateTreeEvaluation, log,
                                    "            unprefer copy to/from #%d, register %d (%s), weight %d, at %s\n",
                                    candidate->getSymbolReference()->getReferenceNumber(),
                                    candidate->getGlobalRegisterNumber(),
                                    self()->getDebug()->getGlobalRegisterName(candidate->getGlobalRegisterNumber()),
                        ...
                        if ((node->getOpCodeValue() == TR::treetop) && (node->getFirstChild()->getOpCode().isCall())) {
                            ...
                            for (uint32_t i = 0; i < callNode->getNumChildren(); i++) {
                                bool isUnpreferred;
                                TR_GlobalRegisterNumber candidateRegister
                                    = self()->findCoalescenceRegisterForParameter(callNode, rc, i, &isUnpreferred);
                                if (candidateRegister != -1) {
                                    if (!isUnpreferred) {
                                        if (remainingRegisters.isSet(candidateRegister)) {
                                            ...
                                            hottestCopiedRegisterWeight = blockWeight;
                                            logprintf(traceSimulateTreeEvaluation, log,
                                                "            found call parameter index %i, register %d (%s), weight "
                                                "%d, at %s\n",
                                                i, hottestCopiedRegister,
                                                self()->getDebug()->getGlobalRegisterName(hottestCopiedRegister),
                                    ...
                                    {
                                        logprintf(traceSimulateTreeEvaluation, log,
                                            "            unprefer call parameter index %i, register %d (%s), weight "
                                            "%d, at %s\n",
                                            i, candidateRegister,
                                            self()->getDebug()->getGlobalRegisterName(candidateRegister), blockWeight,
                                            self()->getDebug()->getName(callNode));
                        ...
                        if (!candidate) {
                            TR::RegisterCandidate *candidate
                                = self()->findUsedCandidate(node, rc, &visitedNodesForCandidateUse);
                            if (candidate) {
                                logprintf(traceSimulateTreeEvaluation, log,
                                    "            unprefer used candidate #%d, register %d (%s), weight %d, at %s\n",
                                    candidate->getSymbolReference()->getReferenceNumber(),
                                    candidate->getGlobalRegisterNumber(),
                                    self()->getDebug()->getGlobalRegisterName(candidate->getGlobalRegisterNumber()),
                                    blockWeight, self()->getDebug()->getName(node));
        ...
        if (hottestCopiedRegister == -1) {
            ...
            if (unpreferredRegisters != remainingRegisters) {
                if (traceSimulateTreeEvaluation && !unpreferredRegisters.isEmpty()) {
                    log->prints("            Rejecting copy registers: ");
                    self()->getDebug()->print(log, &unpreferredRegisters);
                    log->println();
                }
            ...
            logprintf(traceSimulateTreeEvaluation, log, "            Using copy register %d\n", hottestCopiedRegister);
        ...
        if (linkageRegister == -1) {
            ...
            //
            unpreferredRegisters
                = *self()->getGlobalRegisters(TR_linkageSpill, comp->getJittedMethodSymbol()->getLinkageConvention());

            unpreferredRegisters &= remainingRegisters;
            if (unpreferredRegisters != remainingRegisters) {
                if (traceSimulateTreeEvaluation && !unpreferredRegisters.isEmpty()) {
                    log->prints("            Rejecting linkage registers: ");
                    self()->getDebug()->print(log, &unpreferredRegisters);
                    log->println();
                }
                remainingRegisters -= unpreferredRegisters;
            ...
            if (remainingRegisters.isSet(linkageRegister)) {
                logprintf(traceSimulateTreeEvaluation, log, "            Using linkage register %d\n", linkageRegister);
        ...
        if (remainingRegisters.isEmpty()) {
            if (terseSimulateTreeEvaluation)
                log->printf("         } No good registers for candidate #%d after reg pressure simulation\n",
                    rc->getSymbolReference()->getReferenceNumber());
            else
            ...
            while (blockIterator.hasMoreElements()
                && !availableRegisters.isEmpty()) // TODO: Can availableRegisters possibly be empty here?
            {
                TR::Block *block = allBlocks[blockIterator.getNextElement()];
    ...
    if (1) {
        TR::DataType dtype = rc->getDataType();
        uint8_t regsWithheld = (dtype == TR::Float || dtype == TR::Double) ? fprsWithheldFromPickRegister
            : (comp->getMethodHotness() == warm)                           ? gprsWithheldFromPickRegisterWhenWarm
                                                                           : gprsWithheldFromPickRegister;

        if (availableRegisters.elementCount() <= regsWithheld) {
            logprintf(traceGRA, log, "pickRegister: Withholding %d registers from candidate #%d\n",
                availableRegisters.elementCount(), rc->getSymbolReference()->getReferenceNumber());

            return -1;
        } else {
            logprintf(traceGRA, log, "pickRegister: %d registers still available for candidate #%d\n",
                availableRegisters.elementCount(), rc->getSymbolReference()->getReferenceNumber());
        }
        ...
        logprintf(traceGRA, log, "pickRegister: Candidate #%d {\n", rc->getSymbolReference()->getReferenceNumber());
        ...
        if (traceGRA) {
            if (linkageRegister == -1) {
                log->prints("pickRegister:\tNo linkage register\n");
            } else {
                log->printf("pickRegister:\tLinkage register #%d\n", linkageRegister);
            }
        }
        ...
        logprintf(traceGRA, log, "pickRegister:\tPreserved register info is %spresent\n",
        ...
        if (preservedRegisters) {
            ...
            logprintf(traceGRA, log, "pickRegister:\tRegister is %slive across non-cold call\n",
        ...
        logprintf(traceSimulateTreeEvaluation, log, "pickRegister:\tPreference order: %s\n", preferences);
        ...
        for (bestLowRegisterNumber = -1; bestLowRegisterNumber == -1 && preferences[0]; preferences++) {
            switch (preferences[0]) {
                ...
                case 'P': // Any preserved register
                    ...
                    {
                        TR_BitVector regs(availableRegisters);
                        regs &= *preservedRegisters;
                        bestLowRegisterNumber = self()->getFirstBit(regs);
                    }
                    break;

                case 'N': // Any non-preserved register
                    TR_ASSERT(preservedRegisters,
                        "Must have preservedRegisters info to select a non-preserved register");
                    TR_ASSERT(linkageRegister == -1,
                        "Candidates arriving in linkage regs can't be given other linkage regs");
                    //
        ...
        if (traceGRA) {
            if (bestLowRegisterNumber == -1) {
                log->prints("pickRegister:\tNOT assigned a global register\n");
            } else {
                log->printf("pickRegister:\tPicked global register #%d\n", bestLowRegisterNumber);
            }
            log->prints("pickRegister: }\n");
        }
...
TR::RegisterCandidate *OMR::CodeGenerator::findCoalescenceForRegisterCopy(TR::Node *node, TR::RegisterCandidate *rc,
    bool *isUnpreferred)
{
    TR::Compilation *comp = self()->comp();
    TR::RegisterCandidate *candidate = NULL;
    if (node->getOpCode().isStoreDirect() && node->getFirstChild()->getOpCode().isLoadVarDirect()) {
        logprintf(comp->getOption(TR_TraceRegisterPressureDetails), comp->log(), "            found copy %s\n",
            self()->getDebug()->getName(node));

        TR::RegisterCandidate *storedCand = comp->getGlobalRegisterCandidates()->find(node->getSymbolReference());
        ...
        TR::RegisterCandidate *loadedCand
            = comp->getGlobalRegisterCandidates()->find(node->getFirstChild()->getSymbolReference());
        if (loadedCand) {
...
TR_GlobalRegisterNumber OMR::CodeGenerator::findCoalescenceRegisterForParameter(TR::Node *callNode,
    TR::RegisterCandidate *rc, uint32_t childIndex, bool *isUnpreferred)
{
...
TR::RegisterCandidate *OMR::CodeGenerator::findUsedCandidate(TR::Node *node, TR::RegisterCandidate *rc,
...
void OMR::CodeGenerator::computeSimulatedSpilledRegs(TR_BitVector *spilledRegisters, int32_t blockNum,
    TR::SymbolReference *candidate)
{
    ...
    if (symRefNum == _blockAndCandidateRegisterPressureCacheTags[blockNum]) {
        logprintf(traceSimulateTreeEvaluation(), self()->comp()->log(),
...
void OMR::CodeGenerator::computeSpilledRegsForAllPresentLinkages(TR_BitVector *spilledRegisters,
...
void OMR::CodeGenerator::setSpilledRegsForAllPresentLinkages(TR_BitVector *spilledRegisters,
    TR_RegisterPressureSummary &summary, TR_SpillKinds spillKind)
{
...
TR::SymbolReference *OMR::CodeGenerator::TR_RegisterPressureState::getCandidateSymRef()
{
    return _candidate ? _candidate->getSymbolReference() : NULL;
}

bool OMR::CodeGenerator::TR_RegisterPressureState::isInitialized(TR::Node *node)
{
    return node->getVisitCount() == _visitCountForInit;
}
...
{
    _spillMask |= (1 << kind);
    logprintf(cg->traceSimulateTreeEvaluation(), cg->comp()->log(), " !%s", cg->getDebug()->getSpillKindName(kind));
}
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::dumpSpillMask(TR::CodeGenerator *cg)
{
    if (cg->traceSimulateTreeEvaluation()) {
        for (int32_t i = 0; i < TR_numSpillKinds; i++) {
            TR_SpillKinds spillKind = (TR_SpillKinds)i;
            logprintf(isSpilled(spillKind), cg->comp()->log(), " %s", cg->getDebug()->getSpillKindName(spillKind));
        }
    }
}
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::dumpLinkageConventionMask(TR::CodeGenerator *cg)
{
    if (cg->traceSimulateTreeEvaluation()) {
        for (int32_t i = 0; i < TR_NumLinkages; i++) {
            TR_LinkageConventions lc = (TR_LinkageConventions)i;
            logprintf(isLinkagePresent(lc), cg->comp()->log(), " %s", cg->getDebug()->getLinkageConventionName(lc));
        }
    }
}
...
bool OMR::CodeGenerator::isCandidateLoad(TR::Node *node, TR_RegisterPressureState *state)
{
    return state->_candidate ? self()->isCandidateLoad(node, state->_candidate->getSymbolReference()) : false;
}
...
uint8_t OMR::CodeGenerator::gprCount(TR::DataType type, int32_t size)
{
    ...
    return (type.isInt64() && self()->comp()->target().is32Bit() && !self()->use64BitRegsOn32Bit()) ? 2
        : (type.isIntegral() || type.isAddress())                                                   ? 1
                                                                                                    : 0;
}

TR::CodeGenerator::TR_SimulatedNodeState &OMR::CodeGenerator::simulatedNodeState(TR::Node *node,
    ...
    TR::Compilation *comp)
{
    ...
    if (state->_memrefNestDepth >= 2)
        log->printf(" mem*%d", state->_memrefNestDepth);
    else if (state->_memrefNestDepth >= 1)
        log->prints(" mem");
}

inline void leaveSpaceForRegisterPressureState(OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::Compilation *comp)
{
...
static TR::RegisterCandidate *findCandidate(TR::SymbolReference *symRef, TR_LinkHead<TR::RegisterCandidate> *candidates,
    TR::RegisterCandidate *anotherCandidate = NULL)
{
...
static void rememberMostRecentValue(TR::SymbolReference *symRef, TR::Node *valueNode,
    OMR::CodeGenerator::TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
...
static void keepMostRecentValueAliveIfLiveOnEntryToSuccessor(TR::RegisterCandidate *candidate, TR::TreeTop *exitPoint,
    TR::CFGNode *successor, OMR::CodeGenerator::TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    if (candidate->getBlocksLiveOnEntry().isSet(successor->getNumber()) && candidate->getMostRecentValue()) {
...
static void killMostRecentValueIfKeptAliveUntilCurrentTreeTop(TR::RegisterCandidate *candidate,
    OMR::CodeGenerator::TR_RegisterPressureState *state, TR::CodeGenerator *cg)
{
    TR::Compilation *comp = cg->comp();
    if (candidate && candidate->getMostRecentValue()) {
        ...
        if (nodeState._keepLiveUntil == state->_currentTreeTop) {
            ...
            logprintf(comp->getOption(TR_TraceRegisterPressureDetails), comp->log(), " exiting(%s)",
...
void OMR::CodeGenerator::simulateBlockEvaluation(TR::Block *block, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary)
{
    ...
    if (traceSimulateTreeEvaluation) {
        ...
        if (state->_candidate)
            log->printf(", candidate %d", state->getCandidateSymRef()->getReferenceNumber());
        else
            log->prints(", candidate ignored");
        log->prints("\n               Already assigned on entry: ");
        self()->getDebug()->print(log, &state->_alreadyAssignedOnEntry);
        log->prints("\n               Already assigned on exit: ");
        self()->getDebug()->print(log, &state->_alreadyAssignedOnExit);
    }
    ...
    if (state->mustAbort()) {
        logprints(traceSimulateTreeEvaluation, log, "\n               ABORTED");
    ...
    if (1) {
        blockIsLoopHeader = (b = block->getStructureOf()) && (loop = b->getContainingLoop())
    ...
    if (traceSimulateTreeEvaluation) {
        log->printf("\n            } finished simulating block_%d -- g=%d, f=%d, v=%d", block->getNumber(),
            summary->_gprPressure, summary->_fprPressure, summary->_vrfPressure);
        summary->dumpSpillMask(self());
        log->println();
    }
}

void OMR::CodeGenerator::simulationPrePass(TR::TreeTop *tt, TR::Node *node, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary)
{
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        TR::RegisterCandidate *rc
            = findCandidate(node->getSymbolReference(), state->_candidatesAlreadyAssigned, state->_candidate);
        if (rc)
            rc->setLastLoad(node);
        ...
        for (auto e = node->getBlock()->getSuccessors().begin(); e != node->getBlock()->getSuccessors().end(); ++e) {
            ...
            if (state->_candidatesAlreadyAssigned)
                for (TR::RegisterCandidate *candidate = state->_candidatesAlreadyAssigned->getFirst(); candidate;
                     candidate = candidate->getNext())
...
static bool nodeGotFoldedIntoMemref(TR::Node *node, OMR::CodeGenerator::TR_RegisterPressureState *state,
    TR::CodeGenerator *cg)
{
    return (state->_currentMemref && !cg->isCandidateLoad(node, state) && cg->nodeResultGPRCount(node, state) == 0
        && cg->nodeResultFPRCount(node, state) == 0);
}

void OMR::CodeGenerator::simulateTreeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary)
{
    ...
    if (state->mustAbort()) {
        logprints(self()->traceSimulateTreeEvaluation(), log, " ABORTED");
        return;
    }
    ...
    if (self()->getHasResumableTrapHandler()
        && (node->getOpCodeValue() == TR::NULLCHK || node->getOpCodeValue() == TR::ResolveAndNULLCHK)
        && !neverSkipNULLCHKs) {
        ...
        //
        bool isResolvedLoadVar
            = (op.isLoadVar() && child->getFutureUseCount() == 1 && !child->getSymbolReference()->isUnresolved());
        bool canFoldIntoBNDCHK = (nextTreeTop && nextTreeTop->getNode()->getOpCodeValue() == TR::BNDCHK
            && nextTreeTop->getNode()->getFirstChild() == child);
    ...
    //
    if (self()->getSupportsVirtualGuardNOPing() && self()->comp()->performVirtualGuardNOPing()
    ...
    if (nodeState.hasRegister()) {
        ...
        if (self()->nodeWillBeRematerialized(node, state)) {
            // TR_ASSERT(node->getFutureUseCount() >= 2, "Node %s must have refcount >= 2 to be rematerialized",
            ...
            for (uint16_t i = 0; i < node->getNumChildren(); i++) {
                ...
                logprintf(self()->comp()->getOption(TR_TraceRegisterPressureDetails), comp()->log(), " ++%s",
                    self()->getDebug()->getName(child));
            }
            ...
            //
            if (isCall) {
                self()->getNumberOfTemporaryRegistersUsedByCall(node, state, numGPRTemporaries, numFPRTemporaries,
                    numVRFTemporaries);
                if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 || numVRFTemporaries >= 1)
                    && (self()->comp()->getOption(TR_TraceRegisterPressureDetails)
                        && !performTransformation(self()->comp(),
                            "O^O GLOBAL REGISTER ALLOCATION: Call %p has %d GPR and %d FPR and VRF %d dummy "
                            "registers\n",
        ...
        if (state->candidateIsLiveAfterGRA()) {
            ...
            } else if (node->getOpCodeValue() == TR::asynccheck || node->getOpCodeValue() == TR::monent
                || node->getOpCodeValue() == TR::monexit) // TODO: Is this the best test to use?
            {
                summary->spill(TR_vmThreadSpill, self());
            }
        }
...
void OMR::CodeGenerator::simulateSkippedTreeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary, char tagChar)
{
...
void OMR::CodeGenerator::simulateNodeEvaluation(TR::Node *node, TR_RegisterPressureState *state,
    TR_RegisterPressureSummary *summary)
{
    ...
    TR_ASSERT(!self()->simulatedNodeState(node, state).hasRegister(),
        "simulateNodeEvaluation called on live node " POINTER_PRINTF_FORMAT, node);
    ...
    //
    OMR::Logger *log = self()->comp()->log();
    bool traceRegisterPressureDetails = self()->comp()->getOption(TR_TraceRegisterPressureDetails);

    bool evaluateSecondChildFirst = false;
    if (node->getNumChildren() == 2 && !self()->simulatedNodeState(node, state).hasRegister()
    ...
    //
    flags32_t childrenInRegisters(0); // (Don't bother with a bitvector on the slight chance that some child beyond the
                                      // first 32 will get in a register)
    ...
    if (callNodeLinkage) {
        logprintf(traceRegisterPressureDetails, log, " (%s %s linkage)", self()->getDebug()->getName(node),
    ...
    if (evaluateSecondChildFirst) {
        ...
        logprintf(traceRegisterPressureDetails, log, " (%s before %s)",
            self()->getDebug()->getName(node->getSecondChild()), self()->getDebug()->getName(node->getFirstChild()));
        ...
        for (int32_t childIndex = (hasMemRef ? 1 : 0); childIndex < node->getNumChildren(); childIndex++) {
            ...
            if (callNodeLinkage) {
                if (childIndex < maxChildrenInRegisters
                    ...
                    if (callNodeLinkage)
                        logprintf(traceRegisterPressureDetails, log, " (%s arg %d in %s, %d left)",
                    ...
                    if (callNodeLinkage)
                        logprintf(traceRegisterPressureDetails, log, " (%s arg %s in mem)",
                            self()->getDebug()->getName(node), self()->getDebug()->getName(node->getChild(childIndex)));
                }
            }
        }
    ...
    logprintf(traceRegisterPressureDetails, log,
    ...
    if (summary->_gprPressure < summary->PRESSURE_LIMIT)
        TR_ASSERT(unsigned(state->_gprPressure) <= summary->_gprPressure,
            "Children of %s must record max register gprPressure in summary; %d > %d",
            self()->getDebug()->getName(node), state->_gprPressure, summary->_gprPressure);
    if (summary->_fprPressure < summary->PRESSURE_LIMIT)
        TR_ASSERT(unsigned(state->_fprPressure) <= summary->_fprPressure,
            "Children of %s must record max register fprPressure in summary; %d > %d",
            self()->getDebug()->getName(node), state->_fprPressure, summary->_fprPressure);
    if (summary->_vrfPressure <= summary->PRESSURE_LIMIT)
        TR_ASSERT(unsigned(state->_vrfPressure) <= summary->_vrfPressure,
            "Children of %s must record max register vrfPressure in summary; %d > %d",
            self()->getDebug()->getName(node), state->_vrfPressure, summary->_vrfPressure);
    ...
    if (nodeGotFoldedIntoMemref(node, state, self())
        && self()->simulatedNodeState(node)._willBeRematerialized
            == 0 // remat nodes have already had their children incremented, so we must decrement them
    ) {
        ...
        if (callNodeLinkage) {
            ...
            logprintf(traceRegisterPressureDetails, log, " childrenInRegisters=" UINT64_PRINTF_FORMAT_HEX,
    ...
    if (tag)
        logprints(traceRegisterPressureDetails, log, tag);
}
...
bool OMR::CodeGenerator::nodeWillBeRematerialized(TR::Node *node, TR_RegisterPressureState *state)
{
    ...
    if (state->_currentMemref) {
        if (op.isArrayRef())
            ...
            bool isByConst
                = (node->getNumChildren() >= 2) ? (node->getSecondChild()->getOpCode().isLoadConst()) : false;
...
void OMR::CodeGenerator::simulateMemoryReference(TR_SimulatedMemoryReference *memref, TR::Node *node,
    TR_RegisterPressureState *state, TR_RegisterPressureSummary *summary)
{
...
static int64_t powerOfTwoScaleFactor(
    TR::Node *node) // INT_MAX means the node doesn't represent a fixed-point mul by a power of 2
{
    TR::ILOpCode &op = node->getOpCode();
...
bool OMR::CodeGenerator::nodeCanBeFoldedIntoMemref(TR::Node *node, TR_RegisterPressureState *state)
{
    ...
    if ((self()->simulatedNodeState(node).hasRegister() || node->getFutureUseCount() > 1)
        && !self()->nodeWillBeRematerialized(node, state)) {
...
bool OMR::CodeGenerator::nodeResultConsumesNoRegisters(TR::Node *node, TR_RegisterPressureState *state)
{
    TR::ILOpCode &op = node->getOpCode();
    if (op.isTreeTop()) {
...
uint8_t OMR::CodeGenerator::nodeResultGPRCount(TR::Node *node, TR_RegisterPressureState *state)
{
...
uint8_t OMR::CodeGenerator::nodeResultFPRCount(TR::Node *node, TR_RegisterPressureState *state)
{
    if (self()->nodeResultConsumesNoRegisters(node, state))
        return 0;
    else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t OMR::CodeGenerator::nodeResultVRFCount(TR::Node *node, TR_RegisterPressureState *state)
{
    if (self()->nodeResultConsumesNoRegisters(node, state))
        return 0;
    else if (node->getOpCodeValue() == TR::PassThrough)
...
uint8_t OMR::CodeGenerator::nodeResultSSRCount(TR::Node *node, TR_RegisterPressureState *state)
{
...
uint8_t OMR::CodeGenerator::nodeResultARCount(TR::Node *node, TR_RegisterPressureState *state)
{
    if (node->getOpCodeValue() == TR::PassThrough)
...
void OMR::CodeGenerator::getNumberOfTemporaryRegistersUsedByCall(TR::Node *node, TR_RegisterPressureState *state,
    uint32_t &numGPRTemporaries, uint32_t &numFPRTemporaries, uint32_t &numVRFTemporaries)
{
    int32_t numGPRVolatileRegs = 0, numGPRLinkageRegs = 0, numFPRVolatileRegs = 0, numFPRLinkageRegs = 0,
...

    {
        ...
        bv = *self()->getGlobalRegisters(TR_volatileSpill,
            self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
        bv |= *self()->getGlobalRegisters(TR_vmThreadSpill,
            self()->comp()->getJittedMethodSymbol()->getLinkageConvention()); // vmthread, when present, is also a
                                                                              // virtual reg not represented in trees
        ...
        bv &= *self()->getGlobalRegisters(TR_linkageSpill,
            self()->comp()->getJittedMethodSymbol()->getLinkageConvention());
...
void OMR::CodeGenerator::simulateNodeInitialization(TR::Node *node, TR_RegisterPressureState *state)
{
...
void OMR::CodeGenerator::simulateNodeGoingLive(TR::Node *node, TR_RegisterPressureState *state)
{
    TR_ASSERT(state->isInitialized(node), "Node %s should have been initialized before going live",
        self()->getDebug()->getName(node));
    ...
    for (uint16_t i = 0; i < node->getNumChildren(); i++) {
        ...
        if (childNodeState._willBeRematerialized && childNodeState._childRefcountsHaveBeenDecremented) {
            logprints(traceRegisterPressureDetails, log, " rematChild:");
    ...
    if (self()->traceSimulateTreeEvaluation()) {
        ...
        log->printf("%2d(%d) g%+d=%-2d f%+d=%-2d v%+d=%-2d", self()->simulatedNodeState(node, state)._height,
            node->getNumChildren(), self()->nodeResultGPRCount(node, state), state->_gprPressure,
            self()->nodeResultFPRCount(node, state), state->_fprPressure, self()->nodeResultVRFCount(node, state),
            state->_vrfPressure);
        standardNodeSimulationAnnotations(state, self()->comp());
    }
}

void OMR::CodeGenerator::simulateNodeGoingDead(TR::Node *node, TR_RegisterPressureState *state)
{
    OMR::Logger *log = self()->comp()->log();
    bool traceRegisterPressureDetails = self()->comp()->getOption(TR_TraceRegisterPressureDetails);
    ...
    if (self()->isCandidateLoad(node, state) && state->candidateIsLiveOnEntry()
        ...
        logprintf(traceRegisterPressureDetails, log, " *%s", self()->getDebug()->getName(node));
    } else if (self()->isLoadAlreadyAssignedOnEntry(node, state)
...
void OMR::CodeGenerator::simulateDecReferenceCount(TR::Node *node, TR_RegisterPressureState *state)
{
    OMR::Logger *log = self()->comp()->log();
    bool traceRegisterPressureDetails = self()->comp()->getOption(TR_TraceRegisterPressureDetails);
            ...
            if (nodeState._keepLiveUntil) // && !simulatedNodeState(node, state)._willBeRematerialized)
            {
                ...
                if (state->_candidate && node == state->_candidate->getMostRecentValue()) {
                    ...
                    if (!nodeState._isCausingPressureRiskUntilEnd) {
                        ...
                        logprintf(traceRegisterPressureDetails, log, " keep:%s", self()->getDebug()->getName(node));
                    }
                } else
                    logprintf(traceRegisterPressureDetails, log, " keeping:%s",
...
void OMR::CodeGenerator::TR_SimulatedMemoryReference::add(TR::Node *node, TR_RegisterPressureState *state,
    TR::CodeGenerator *cg)
{
    OMR::Logger *log = cg->comp()->log();
    bool traceRegisterPressureDetails = cg->comp()->getOption(TR_TraceRegisterPressureDetails);

    if (_numRegisters >= MAX_NUM_REGISTERS) {
        // Pretend to emit an LEA
        logprints(traceRegisterPressureDetails, log, " consolidateMemref{");

        simulateDecNodeReferenceCounts(state, cg);
        _numConsolidatedRegisters = 1;
        state->_gprPressure += _numConsolidatedRegisters;

        logprints(traceRegisterPressureDetails, log, " }");
    }
    //_undecrementedNodes.add(node);
    _numRegisters += 1;
    //_numRegisters += cg->simulatedNodeState(node)._liveGPRs; // Fix this once we find a platform that can put other
    // reg kinds in a memref
}

void OMR::CodeGenerator::TR_SimulatedMemoryReference::simulateDecNodeReferenceCounts(TR_RegisterPressureState *state,
    TR::CodeGenerator *cg)
{
...
void OMR::CodeGenerator::TR_RegisterPressureSummary::accumulate(TR_RegisterPressureState *state, TR::CodeGenerator *cg,
    uint32_t gprTemps, uint32_t fprTemps, uint32_t vrfTemps)
{
