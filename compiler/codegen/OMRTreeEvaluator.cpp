...
TR::Register *OMR::TreeEvaluator::brdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::TreeEvaluator::srdbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::srdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::irdbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::irdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::lrdbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::lrdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::ardbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::ardbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::frdbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::frdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::drdbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The direct rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getFirstChild();
...
TR::Register *OMR::TreeEvaluator::drdbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The indirect rdbar IL represents a load with side effects.
    // Currently we don't use the side effect node (first child), so we delegate
    // evaluation to the load evaluator (which will evaluate the first child as well).
...
TR::Register *OMR::TreeEvaluator::bwrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::TreeEvaluator::bwrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
...
TR::Register *OMR::TreeEvaluator::swrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getSecondChild();
...
TR::Register *OMR::TreeEvaluator::swrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getThirdChild();
...
TR::Register *OMR::TreeEvaluator::iwrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getSecondChild();
...
TR::Register *OMR::TreeEvaluator::iwrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getThirdChild();
...
TR::Register *OMR::TreeEvaluator::lwrtbarEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getSecondChild();
...
TR::Register *OMR::TreeEvaluator::lwrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    // The wrtbar IL op represents a store with side effects.
    // Currently we don't use the side effect node. So just evaluate it and decrement the reference count.
    TR::Node *sideEffectNode = node->getThirdChild();
...
TR_GlobalRegisterNumber OMR::TreeEvaluator::getHighGlobalRegisterNumberIfAny(TR::Node *node, TR::CodeGenerator *cg)
{
...
int32_t OMR::TreeEvaluator::checkNonNegativePowerOfTwo(int32_t value)
{
...
int32_t OMR::TreeEvaluator::checkNonNegativePowerOfTwo(int64_t value)
{
    if (isNonNegativePowerOf2(value)) {
        int32_t shiftAmount = 0;
...
TR::Register *OMR::TreeEvaluator::compressedRefsEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
TR::Register *OMR::TreeEvaluator::computeCCEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
...
bool OMR::TreeEvaluator::nodeIsIAddOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
    ...
    static TR_Pattern *pattern = pb.choice(pb(TR::ificmpge), pb(TR::ificmplt),
        pb.children(pb(TR::iand,
                        pb(TR::ixor, pb(TR::iadd, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
                            pb(u, u->leftChild)),
                        pb(TR::ixor, pb(u, u->operationNode), pb(u, u->rightChild))),
            pb.iconst(0)));
...
bool OMR::TreeEvaluator::nodeIsISubOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
    ...
    static TR_Pattern *pattern = pb.choice(pb(TR::ificmpge), pb(TR::ificmplt),
        pb.children(pb(TR::iand,
                        pb(TR::ixor, pb(TR::isub, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
                            pb(u, u->leftChild)),
                        pb(TR::ixor, pb(u, u->leftChild), pb(u, u->rightChild))),
            pb.iconst(0)));
    return pattern->matches(node, (TR::Node **)u, comp);
}
...
bool OMR::TreeEvaluator::nodeIsLAddOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
    ...
    TR_PatternBuilder pb(comp);
    static TR_Pattern *pattern = pb.choice(pb(TR::iflcmpge), pb(TR::iflcmplt),
        pb.children(pb(TR::land,
                        pb(TR::lxor, pb(TR::ladd, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
                            pb(u, u->leftChild)),
                        pb(TR::lxor, pb(u, u->operationNode), pb(u, u->rightChild))),
            pb.lconst(0)));
    return pattern->matches(node, (TR::Node **)u, comp);
}
...
bool OMR::TreeEvaluator::nodeIsLSubOverflowCheck(TR::Node *node, TR_ArithmeticOverflowCheckNodes *u)
{
    ...
    static TR_Pattern *pattern = pb.choice(pb(TR::iflcmpge), pb(TR::iflcmplt),
        pb.children(pb(TR::land,
                        pb(TR::lxor, pb(TR::lsub, u, u->operationNode, pb(u, u->leftChild), pb(u, u->rightChild)),
                            pb(u, u->leftChild)),
                        pb(TR::lxor, pb(u, u->leftChild), pb(u, u->rightChild))),
            pb.lconst(0)));
    return pattern->matches(node, (TR::Node **)u, comp);
}
...
void OMR::TreeEvaluator::evaluateNodesWithFutureUses(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (node->getFutureUseCount() > 0) {
        ...
        while (actualLoadOrStoreChild->getOpCode().isConversion() || actualLoadOrStoreChild->chkCompressionSequence())
