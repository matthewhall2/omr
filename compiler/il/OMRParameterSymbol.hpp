...
public:
    template<typename AllocatorType> static TR::ParameterSymbol *create(AllocatorType, TR::DataType, int32_t);

    template<typename AllocatorType> static TR::ParameterSymbol *create(AllocatorType, TR::DataType, int32_t, size_t);
    ...
    void setTypeSignature(const char *s, int32_t l)
    {
        _typeLength = l;
        _typeSignature = s;
    }

    const char *getTypeSignature(int32_t &len)
    {
        len = _typeLength;
