...
public:
    ...
    ValuePropagationPtr operator[](TR::ILOpCode opcode) const { return table[opcode.getTableIndex()]; }
};
...
typedef std::map<TR::CFGEdge *, TR_BitVector *, std::less<TR::CFGEdge *>, DefinedOnAllPathsMapAllocator>
    DefinedOnAllPathsMap;

typedef TR::typed_allocator<std::pair<TR::Node * const, List<TR_Pair<TR::TreeTop, TR::CFGEdge> > *>, TR::Region &>
    CallNodeToGuardNodesMapAllocator;
typedef std::map<TR::Node *, List<TR_Pair<TR::TreeTop, TR::CFGEdge> > *, std::less<TR::Node *>,
    CallNodeToGuardNodesMapAllocator>
    CallNodeToGuardNodesMap;
...
private:
    TR::TreeTop *createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef, TR::SymbolReference *lenRef, TR::SymbolReference *srcObjRef,
        TR::SymbolReference *dstObjRef, bool isForward);
    TR::TreeTop *createArrayNode(TR::TreeTop *tree, TR::TreeTop *newTree, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef, TR::Node *lenRef, TR::SymbolReference *srcObjRef, TR::SymbolReference *dstObjRef,
        bool isForward);
    ...
    TR::TreeTop *createRangeCompareNode(TR::Node *node, TR::SymbolReference *srcRef, TR::SymbolReference *dstRef,
    ...
    bool hasChangedTrees() { return _changedTrees; }

    void setChangedTrees(bool state) { _changedTrees = state; }
...
public:
    ...
    ValueConstraint *createValueConstraint(int32_t valueNumber, Relationship *relationships,
        StoreRelationship *storeRelationships);
    ...
    TR::VPConstraint *addGlobalConstraint(TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint,
        int32_t relative);
    TR::VPConstraint *addBlockConstraint(TR::Node *node, TR::VPConstraint *constraint, TR::Node *relative = NULL,
        bool mustBeValid = true);
    TR::VPConstraint *addEdgeConstraint(TR::Node *node, TR::VPConstraint *constraint, EdgeConstraints *contraints,
        TR::Node *relative = NULL);
    TR::VPConstraint *addConstraintToList(TR::Node *node, int32_t valueNumber, int32_t relative,
        TR::VPConstraint *constraint, ValueConstraints *valueConstraints, bool replaceExisting = false);
    TR::VPConstraint *addBlockOrGlobalConstraint(TR::Node *node, TR::VPConstraint *constraint, bool isGlobal,
        TR::Node *relative = NULL);
    void mergeRelationships(TR_LinkHead<Relationship> &fromList, TR_LinkHead<Relationship> &toList, int32_t valueNumber,
        bool preserveFrom = false, StoreRelationship *mergingStore = NULL, List<TR::Symbol> *storeSymbols = NULL,
    ...
    void removeConstraint(int32_t valueNumber, ValueConstraints &valueConstraints,
        int32_t relative = AbsoluteConstraint);
    ...
    Relationship *findValueConstraint(int32_t valueNumber, ValueConstraints &valueConstraints,
        int32_t relative = AbsoluteConstraint);
    ...
    StoreRelationship *findStoreValueConstraint(int32_t valueNumber, TR::Symbol *symbol,
        ValueConstraints &valueConstraints);
    ...
    TR::VPConstraint *applyGlobalConstraints(TR::Node *node, int32_t valueNumber, TR::VPConstraint *constraint,
        int32_t relative);
    void invalidateParmConstraintsIfNeeded(TR::Node *node, TR::VPConstraint *constraint);
    void checkTypeRelationship(TR::VPConstraint *lhs, TR::VPConstraint *rhs, int32_t &value, bool isInstanceOf,
        bool isCheckCast);
    ...
    bool propagateConstraint(TR::Node *node, int32_t valueNumber, Relationship *first, Relationship *rel,
        ValueConstraints *valueConstraints);
    ...
    struct VPTreeTopPair {
        ...
        VPTreeTopPair(TR::TreeTop *treetop1, TR::TreeTop *treetop2)
            : _treetop1(treetop1)
            , _treetop2(treetop2)
        {}
    ...
    struct TR_ArrayCopySpineCheck {
        ...
        TR_ArrayCopySpineCheck(TR::TreeTop *arraycopyTree, TR::SymbolReference *arraycopySymRef, TR::Node *srcObjNode,
            TR::Node *srcOffNode, TR::Node *dstObjNode, TR::Node *dstOffNode, TR::Node *copyLenNode)
    ...
    struct TR_NeedRuntimeTestNullRestrictedArrayCopy {
        ...
        TR_NeedRuntimeTestNullRestrictedArrayCopy(TR::SymbolReference *dstArrRefSymRef,
            TR::SymbolReference *srcArrRefSymRef, TR::TreeTop *ptt, TR::TreeTop *ntt, TR::Block *originBlock,
            TR::Block *slowBlock, bool testDstArray)
            : _dstArrRefSymRef(dstArrRefSymRef)
            , _srcArrRefSymRef(srcArrRefSymRef)
            , _prevTT(ptt)
            , _nextTT(ntt)
            , _originBlock(originBlock)
            , _slowBlock(slowBlock)
            , _needRuntimeTestDstArray(testDstArray)
        {}
    ...
    void transformReferenceArrayCopyWithoutCreatingStoreTrees(TR_TreeTopWrtBarFlag *arrayTree,
    ...
    TR::TreeTop *createPrimitiveArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
        TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
        bool useFlagsOnOriginalArraycopy, bool isOptimizedReferenceArraycopy);
    TR::TreeTop *createReferenceArrayNodeWithoutFlags(TR::TreeTop *tree, TR::TreeTop *newTree,
...
#ifdef J9_PROJECT_SPECIFIC
    void generateArrayTranslateNode(TR::TreeTop *vcallTree, TR::TreeTop *arrayTranslateTree,
...
#endif
    TR::TreeTop *createConverterCallNodeAfterStores(TR::TreeTop *tree, TR::TreeTop *origTree,
        TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *lenRef,
    ...
    TR::TreeTop *createArrayCopyVCallNodeAfterStores(TR::TreeTop *tree, TR::SymbolReference *srcObjectRef,
        TR::SymbolReference *dstObjectRef, TR::SymbolReference *lenRef, TR::SymbolReference *srcRef,
        TR::SymbolReference *dstRef);
    TR::TreeTop *createSameLeafTestAfterStores(TR::TreeTop *tree, TR::SymbolReference *child1Ref,
        TR::SymbolReference *child2Ref);
    void generateRTArrayNodeWithoutFlags(TR_RealTimeArrayCopy *rtArrayCopyTree, TR::TreeTop *dupArraycopyTree,
        TR::SymbolReference *srcRef, TR::SymbolReference *dstRef, TR::SymbolReference *srcOffRef,
        TR::SymbolReference *dstOffRef, TR::SymbolReference *lenRef, bool primitive);
    ...
    struct InductionVariable : public TR_Link<InductionVariable> {
        InductionVariable(TR::Symbol *sym, TR::Node *entryDef, int32_t incrVN, TR::VPConstraint *incr,
    ...
    struct VirtualGuardInfo : public TR_Link<VirtualGuardInfo> {
        VirtualGuardInfo(OMR::ValuePropagation *vp, TR_VirtualGuard *vgOld, TR_VirtualGuard *vgNew, TR::Node *newGNode,
            TR::Node *cn);
...
void constrainRangeByPrecision(const int64_t low, const int64_t high, const int32_t precision, int64_t &lowResult,
    int64_t &highResult, bool isNonNegative = false);
