...
extern bool storeHelperImmediateInstruction(TR::Node *valueChild, TR::CodeGenerator *cg, bool isReversed,
...
OMR::Z::Linkage::Linkage(TR::CodeGenerator *cg)
    : OMR::Linkage(cg)
    , _linkageType(TR_None)
    , _raContextSaveNeeded(true)
...
TR::Instruction *OMR::Z::Linkage::loadUpArguments(TR::Instruction *cursor) { return NULL; }

void OMR::Z::Linkage::removeOSCOnSavedArgument(TR::Instruction *instr, TR::Register *sReg, int32_t stackOffset)
{
    bool done = false;
    ...
    while ((current != NULL) && !current->isLabel() && !current->isCall() && !current->isBranchOp()
        ...
        if (current->isLoad()) {
            ...
            if (disp == stackOffset) {
                ...
                if (opCode == TR::InstOpCode::L) {
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            ...
                            "=> [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LG %s,%d(SP) to TR::InstOpCode::LGR %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LE %s,%d(SP) to TR::InstOpCode::LER %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LD %s,%d(SP) to TR::InstOpCode::LDR %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LT %s,%d(SP) to TR::InstOpCode::LTR %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LTG %s,%d(SP) to TR::InstOpCode::LTGR %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                    ...
                    if (self()->comp()->getOption(TR_TraceCG)) {
                        traceMsg(self()->comp(),
                            "\nProlog peeking changing TR::InstOpCode::LGF %s,%d(SP) to TR::InstOpCode::LGFR %s,%s : "
                            "[%p] => [%p]\n",
                            tReg->getRegisterName(self()->comp()), stackOffset, tReg->getRegisterName(self()->comp()),
                            sReg->getRegisterName(self()->comp()), current, newInst);
                    }
                ...
                //
                if (newInst && performTransformation(self()->comp(), "O^O : Prolog peeking removing [%p]\n", current)) {
 ...
 * the prolog code.
 */
void *OMR::Z::Linkage::saveArguments(void *cursor, bool genBinary, bool InPreProlog, int32_t frameOffset,
    List<TR::ParameterSymbol> *parameterList)
{
    ...
    for (i1 = TR::RealRegister::FirstGPR; i1 <= lastReg; i1++) {
        ...
        if ((gpr6Test || !self()->getPreserved(REGNUM(i1))) && !self()->getIntegerArgument(REGNUM(i1))
    ...
    if (self()->isSkipGPRsForFloatParms()) { // set GPRs skipped for FPRs to free
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (self()->isSmallIntParmsAlignedRight()) {
            // On both Linux and z/OS system linkages we spill both the clean and dirty bits of the incoming arguments
            // to the stack if needed. The `initParamOffset` API will ensure the parameters within the method are
            // initialized with offsets which point only to the clean data (no dirty bits loaded).
            slotSize = gprSize;
        }
        ...
        if (((self()->isSmallIntParmsAlignedRight() && paramCursor->getType().isIntegral())
                || (self()->isPadFloatParms() && paramCursor->getType().isFloatingPoint()))
            && (gprSize > paramCursor->getSize())) {
        ...
        if (lri >= 0) {
            ...
            if (type.isFloatingPoint()) // float/double/long double
            {
                ...
                TR_ASSERT(paramCursor->getSize() <= 8,
                    "Only aggregates of size 8 bytes or less are passed in registers");
            ...
            // This query here is actually not ideal.  originally isReferencedParameter (which actually means
            // isReferencedParameterAtIlGenTime) was replaced with isParmHasToBeOnStack, which would make sense.
            ...
            if ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor)
                ...
                switch (dtype) {
                    ...
                    case TR::Aggregate: // Should only happen on zLinux
                    {
                        if (genBinary) {
                            ...
                            TR::MemoryReference *mr
                                = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);

                            if (storeOpCode == TR::InstOpCode::STCM)
                                cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::STCM, firstNode,
                                    self()->getRealRegister(regNum), opcodeMask, mr, (TR::Instruction *)cursor);

                            else
                                cursor = generateRXInstruction(self()->cg(), storeOpCode, firstNode,
                                    self()->getRealRegister(regNum), mr, (TR::Instruction *)cursor);

                            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
                                    self()->getRealRegister(regNum), offset);
                        }
                    }

                        if (secondStore && fullLong && self()->cg()->comp()->target().is32Bit()) {
                            if (genBinary) {
                                cursor = (void *)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::ST,
                                    (uint8_t *)cursor, self()->getRealRegister(regNum), offset, self()->cg());
                            } else {
                                TR::MemoryReference *mr
                                    = generateS390MemoryReference(stackPtr, offset + 4, self()->cg(), param_name);
                                cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::ST, firstNode,
                                    self()->getRealRegister(REGNUM(regNum + 1)), mr, (TR::Instruction *)cursor);
                                ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                                if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                    self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
                                        self()->getRealRegister(regNum), offset);
                    ...
                    case TR::Float:
                        if (genBinary) {
                            cursor = (void *)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::STE,
                                (uint8_t *)cursor, self()->getRealRegister(regNum), offset, self()->cg());
                        } else {
                            TR::MemoryReference *mr
                                = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                            cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::STE, firstNode,
                                self()->getRealRegister(regNum), mr, (TR::Instruction *)cursor);
                            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
                    ...
                    case TR::Double:
                        if (genBinary) {
                            cursor = (void *)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD,
                                (uint8_t *)cursor, self()->getRealRegister(regNum), offset, self()->cg());
                        } else {
                            TR::MemoryReference *mr
                                = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                            cursor = (void *)generateRXInstruction(self()->cg(), TR::InstOpCode::STD, firstNode,
                                self()->getRealRegister(regNum), mr, (TR::Instruction *)cursor);
                            ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
                    ...
                    default:
                        if (dtype.isVector()) {
                            ...
                            if (elementType == TR::Int8 || elementType == TR::Int16 || elementType == TR::Int32
                                || elementType == TR::Int64 || elementType == TR::Double) {
                                if (genBinary) {
                                    cursor = (void *)TR::S390CallSnippet::storeArgumentItem(TR::InstOpCode::VST,
                                        (uint8_t *)cursor, self()->getRealRegister(regNum), offset, self()->cg());
                                } else {
                                    TR::MemoryReference *mr
                                        = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                                    cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::VST, firstNode,
                                        self()->getRealRegister(regNum), mr, (TR::Instruction *)cursor);
                                    ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                                    if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                        self()->removeOSCOnSavedArgument((TR::Instruction *)cursor,
                ...
                if (regNum != ai || (dtype == TR::Int64 && self()->cg()->comp()->target().is32Bit())) {
                    ...
                    if (freeScratchable.isSet(ai)) {
                        if (dtype == TR::Float || dtype == TR::Double) {
                            cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LDR, firstNode,
                                self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(regNum),
                            ...
                            if (dtype == TR::Int64 && self()->cg()->comp()->target().is32Bit()) {
                                cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLG, firstNode,
                                    self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(regNum), 32,
                                    (TR::Instruction *)cursor);
                                ...
                                if (fullLong) {
                                    cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LR, firstNode,
                                        self()->getRealRegister(REGNUM(ai)),
                                        self()->getRealRegister(REGNUM(regNum + 1)), (TR::Instruction *)cursor);

                                    freeScratchable.set(regNum + 1);
                                } else {
                                    cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::L, firstNode,
                                        self()->getRealRegister(REGNUM(ai)),
                                        generateS390MemoryReference(stackPtr, offset + 4, self()->cg()),
                                        (TR::Instruction *)cursor);
                                }
                                ...
                                cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(),
                                    firstNode, self()->getRealRegister(REGNUM(ai)), self()->getRealRegister(regNum),
                                    (TR::Instruction *)cursor);
            ...
            if (ai_l >= 0 && !InPreProlog && !skipLong) {
                ...
                //  - complex:  can pass real part in FP reg (or FP reg pair for complex long double) and imag part on
                //  stack
                ...
                //
                    ...
                    if (freeScratchable.isSet(ai_l)) {
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                            self()->getRealRegister(REGNUM(ai_l)), self()->getRealRegister(REGNUM(regNum + 1)),
                            (TR::Instruction *)cursor);
        ...
        //
            ...
            switch (dtype) {
                ...
                case TR::Aggregate: // Should only happen on zLinux
                    if (freeScratchable.isSet(ai)) {
                        ...
                        if (loadOpCode == TR::InstOpCode::ICM)
                            cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM, firstNode,
                                self()->getRealRegister(REGNUM(ai)), opcodeMask, mr, (TR::Instruction *)cursor);
                        else
                            cursor = generateRXInstruction(self()->cg(), loadOpCode, firstNode,
                                self()->getRealRegister(REGNUM(ai)), mr, (TR::Instruction *)cursor);
                ...
                case TR::Float:
                    if (freeScratchable.isSet(ai)) {
                        cursor = generateRXEInstruction(self()->cg(), TR::InstOpCode::LDE, firstNode,
                            self()->getRealRegister(REGNUM(ai)),
                            generateS390MemoryReference(stackPtr, offset, self()->cg()), 0,
                            static_cast<TR::Instruction *>(cursor));
                        ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                ...
                case TR::Double:
                    if (freeScratchable.isSet(ai)) {
                        cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, firstNode,
                            self()->getRealRegister(REGNUM(ai)),
    ...
    while (freeMore && numMoves > 0) {
        ...
        for (i1 = 0; i1 < busyIndex; i1++) {
            ...
            if (!(target < 0)
                && (freeScratchable.isSet(target) || (source == target && busyMoves[2][i1] == 6)
                    || (source == target && busyMoves[2][i1] == 7))) {
                switch (busyMoves[2][i1]) {
                    case 0: // Reg 2 Reg
                    {
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                            self()->getRealRegister(REGNUM(target)), self()->getRealRegister(REGNUM(source)),
                            (TR::Instruction *)cursor);
                    ...
                    case 1: // Int or Addr  Reg from Stack Slot
                    {
                        ...
                        if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
                            cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM, firstNode,
                                self()->getRealRegister(REGNUM(target)), busyMoves[4][i1], mr,
                                (TR::Instruction *)cursor);

                        else if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::LFH)
                            cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LFH, firstNode,
                                self()->getRealRegister(REGNUM(target)), mr, (TR::Instruction *)cursor);

                        else
                            cursor = generateRXInstruction(self()->cg(), (TR::InstOpCode::Mnemonic)busyMoves[3][i1],
                                firstNode, self()->getRealRegister(REGNUM(target)), mr, (TR::Instruction *)cursor);
                    ...
                    case 3: // Floats
                        cursor = generateRXEInstruction(self()->cg(), TR::InstOpCode::LDE, firstNode,
                            self()->getRealRegister(REGNUM(target)),
                            generateS390MemoryReference(stackPtr, source, self()->cg()), 0,
                            static_cast<TR::Instruction *>(cursor));
                        ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                        break;
                    case 4: // Doubles
                    ...
                    case 6: // 64bit reg on 31bit
                        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLG, firstNode,
                            self()->getRealRegister(REGNUM(target)), self()->getRealRegister(REGNUM(source)), 32,
                            (TR::Instruction *)cursor);
                    ...
                    case 9: // LongDouble
                        ...
                        ((TR::Instruction *)cursor)->setBinLocalFreeRegs(binLocalRegs);
                        cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::LD, firstNode,
                            self()->getRealRegister(REGNUM(target + 2)),
                    ...
                    case 10: // LongDouble
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                            self()->cg()->allocateFPRegisterPair(self()->getRealRegister(REGNUM(target + 2)),
                                self()->getRealRegister(REGNUM(target))),
                            self()->cg()->allocateFPRegisterPair(self()->getRealRegister(REGNUM(source + 2)),
                                self()->getRealRegister(REGNUM(source))),
                            (TR::Instruction *)cursor);
                        freeScratchable.set(source);
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node *child, bool isStore, bool isRegReg)
{
    ...
    switch (child->getDataType()) {
        case TR::Int64:
            ...
            break;
        case TR::Address:
            return (isStore) ? TR::InstOpCode::getStoreOpCode()
                : (isRegReg) ? TR::InstOpCode::getLoadRegOpCode()
        ...
        case TR::Int32:
            if (self()->cg()->comp()->target().is32Bit())
                ...
                return (isStore) ? (isWiden) ? TR::InstOpCode::STG : TR::InstOpCode::ST
                    : (isRegReg) ? (isWiden) ? TR::InstOpCode::LGFR : TR::InstOpCode::LGR
                    : (isWiden)  ? TR::InstOpCode::LGF
                                 : TR::InstOpCode::L;
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getRegCopyOpCodeForLinkage(TR::Node *child)
{
    return self()->getOpCodeForLinkage(child, false, true);
}

TR::InstOpCode::Mnemonic OMR::Z::Linkage::getStoreOpCodeForLinkage(TR::Node *child)
{
    return self()->getOpCodeForLinkage(child, true, false);
}

TR::InstOpCode::Mnemonic OMR::Z::Linkage::getLoadOpCodeForLinkage(TR::Node *child)
{
    return self()->getOpCodeForLinkage(child, false, false);
}

TR::Register *OMR::Z::Linkage::copyArgRegister(TR::Node *callNode, TR::Node *child, TR::Register *argRegister)
{
    ...
    if (self()->cg()->comp()->target().is32Bit() && child->getDataType() == TR::Int64
        && !argRegister->getRegisterPair()) {
        TR::Register *tempRegH = self()->cg()->allocateRegister();
        TR::Register *tempRegL = self()->cg()->allocateRegister();

        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SRLG, callNode, tempRegH, argRegister, 32);
        if (debugObj) {
            debugObj->addInstructionComment(toS390RSInstruction(cursor), REG_PARAM);
        }

        cursor = generateRRInstruction(self()->cg(), copyOpCode, callNode, tempRegL, argRegister);
        if (debugObj) {
...
TR::Register *OMR::Z::Linkage::pushLongArg32(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies,
 ...
 */
TR::Register *OMR::Z::Linkage::getStackRegisterForOutgoingArguments(TR::Node *n,
    TR::RegisterDependencyConditions *dependencies)
{
    ...
    if (!self()->isFastLinkLinkageType()) { // normal case
        sreg = stackRegister;
    }
    return sreg;
}

TR::Register *OMR::Z::Linkage::pushArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies,
    TR::Register *argRegister)
{
    ...
    if (argType.isInt64() || argDataType == TR::Double) {
        argSize += self()->isTwoStackSlotsForLongAndDouble() ? 2 * regSize : 8;
    } else
        argSize += regSize;

    if (self()->cg()->comp()->target().is32Bit() && argType.isInt64())
        return self()->pushLongArg32(callNode, child, numIntegerArgs, numFloatArgs, stackOffsetPtr, dependencies,
            argRegister);
    ...
    if (isFloatShadowedToIntRegs) {
        TR::Register *stackRegister = self()->getStackRegisterForOutgoingArguments(callNode,
            dependencies); // delay (possibly) creating this till needed
        self()->loadIntArgumentsFromStack(callNode, dependencies, argDataType, (*stackOffsetPtr) - argSize, argSize,
            numIntegerArgs, stackRegister);
    }

    return argRegister;
}

TR::Register *OMR::Z::Linkage::pushJNIReferenceArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies,
    TR::Register *argRegister)
{
...
TR::Register *OMR::Z::Linkage::pushVectorArg(TR::Node *callNode, TR::Node *child, int32_t numVectorArgs, int32_t pindex,
    int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies, TR::Register *argRegister)
{
    TR::DataType argType = child->getType();
...
TR::Register *OMR::Z::Linkage::pushAggregateArg(TR::Node *callNode, TR::Node *child, int32_t numIntegerArgs,
    int32_t numFloatArgs, int32_t *stackOffsetPtr, TR::RegisterDependencyConditions *dependencies,
    TR::Register *argRegister)
{
#ifdef J9_PROJECT_SPECIFIC
 ...
 * canonical positions
 */
void OMR::Z::Linkage::loadIntArgumentsFromStack(TR::Node *callNode, TR::RegisterDependencyConditions *dependencies,
    TR::DataType argType, int32_t stackOffset, int32_t argSize, int32_t numIntegerArgs, TR::Register *stackRegister)
{
    ...
    int64_t killMask, TR::Register *&vftReg, bool PassReceiver, bool isRightToLeft)
{
    ...
    bool rightToLeft = self()->isParmsInReverseOrder() &&
        ...
        !callNode->getSymbolReference()->isOSRInductionHelper() && isRightToLeft;
        ...
        switch (child->getDataType()) {
            case TR::Address:
                if (isFastJNI && child->getDataType() == TR::Address) {
                    argRegister = self()->pushJNIReferenceArg(callNode, child, numIntegerArgs, numFloatArgs,
            ...
            case TR::Int64:
                if (self()->cg()->comp()->target().is32Bit()) {
                    argRegister = self()->pushLongArg32(callNode, child, numIntegerArgs, numFloatArgs, &stackOffset,
                        dependencies);
                    numIntegerArgs += 2;
                } else {
                    argRegister
            ...
            case TR::Aggregate:
                if (self()->isAggregatesPassedInParmRegs() || self()->isAggregatesPassedOnParmStack()) {
                    argRegister = self()->pushAggregateArg(callNode, child, numIntegerArgs, numFloatArgs, &stackOffset,
                        dependencies);
            ...
            default:
                if (child->getDataType().isVector()) {
                    ...
                    if (elementType == TR::Int8 || elementType == TR::Int16 || elementType == TR::Int32
                        || elementType == TR::Int64 || elementType == TR::Double) {
                        argRegister
                            = self()->pushVectorArg(callNode, child, numVectorArgs, i, &stackOffset, dependencies);
                        numVectorArgs++;
                        break;
                    }
                }
    ...
    switch (resDataType) {
        ...
        case TR::Int64:
            if (self()->cg()->comp()->target().is32Bit()) {
                ...
                dependencies->addPostCondition(resultRegLow, self()->getLongLowReturnRegister(),
                    DefinesDependentRegister);
                ...
                dependencies->addPostCondition(resultRegHigh, self()->getLongHighReturnRegister(),
                    DefinesDependentRegister);
        ...
        case TR::Int16:
        case TR::Int32:
            resultReg = (resType.isAddress()) ? self()->cg()->allocateCollectedReferenceRegister()
        ...
        default:
            if (resDataType.isVector()) {
                ...
                if (elementType == TR::Int8 || elementType == TR::Int16 || elementType == TR::Int32
                    ...
                    dependencies->addPostCondition(resultReg, self()->getVectorReturnRegister(),
                        DefinesDependentRegister);
...
void OMR::Z::Linkage::replaceCallWithJumpInstruction(TR::Instruction *callInstruction)
{
    ...
    TR_ASSERT(opCode == TR::InstOpCode::BASSM || opCode == TR::InstOpCode::BASR || opCode == TR::InstOpCode::BALR
            || opCode == TR::InstOpCode::BAS || opCode == TR::InstOpCode::BAL || opCode == TR::InstOpCode::BRAS
            || opCode == TR::InstOpCode::BRASL,
        "Wrong opcode type on instruction!\n");
...
TR::Instruction *OMR::Z::Linkage::storeArgumentOnStack(TR::Node *callNode, TR::InstOpCode::Mnemonic opCode,
    TR::Register *argReg, int32_t *stackOffsetPtr, TR::Register *stackRegister)
{
    TR::Instruction *cursor;
    int32_t padding = (self()->isPadFloatParms() && opCode == TR::InstOpCode::STE) ? 4 : 0;
    int32_t storeOffset = *stackOffsetPtr;

    TR::MemoryReference *argMemRef = generateS390MemoryReference(stackRegister, storeOffset + padding, self()->cg());
 ...
 */
TR::Instruction *OMR::Z::Linkage::storeLongDoubleArgumentOnStack(TR::Node *callNode, TR::DataType argType,
...
int64_t OMR::Z::Linkage::killAndAssignRegister(int64_t killMask, TR::RegisterDependencyConditions *deps,
    TR::Register **virtualRegPtr, TR::RealRegister::RegNum regNum, TR::CodeGenerator *cg, bool isAllocate, bool isDummy)
{
...
int64_t OMR::Z::Linkage::killAndAssignRegister(int64_t killMask, TR::RegisterDependencyConditions *deps,
    TR::Register **virtualRegPtr, TR::RealRegister *realReg, TR::CodeGenerator *cg, bool isAllocate, bool isDummy)
{
    return self()->killAndAssignRegister(killMask, deps, virtualRegPtr, realReg->getRegisterNumber(), cg, isAllocate,
        isDummy);
}

void OMR::Z::Linkage::generateDispatchReturnLable(TR::Node *callNode, TR::CodeGenerator *cg,
    TR::RegisterDependencyConditions *&deps, TR::Register *javaReturnRegister, bool hasGlRegDeps,
    TR::Node *GlobalRegDeps)
{
    ...
    if (javaReturnRegister) {
        deps->stopUsingDepRegs(self()->cg(),
            javaReturnRegister->getRegisterPair() == NULL ? javaReturnRegister : javaReturnRegister->getHighOrder(),
            javaReturnRegister->getLowOrder());
    } else {
        deps->stopUsingDepRegs(self()->cg());
    }
...
void OMR::Z::Linkage::setupRegisterDepForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
    ...
    TR::Node *&GlobalRegDeps, bool &hasGlRegDeps, TR::Register **methodAddressReg, TR::Register *&JavaLitOffsetReg)
{
    ...
    if (!self()->cg()->comp()->target().isZOS()) {
        ...
        killMask = self()->killAndAssignRegister(killMask, deps, &parm3VirtualRegister, TR::RealRegister::GPR4,
            self()->cg(), true);
    }

    if (dispatchType == TR_SystemDispatch) {
        if (!self()->cg()->comp()->target().isZOS())
            killMask = self()->killAndAssignRegister(killMask, deps, methodAddressReg, TR::RealRegister::GPR14,
                self()->cg(), true);
    }
}

void OMR::Z::Linkage::setupBuildArgForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
...
void OMR::Z::Linkage::performCallNativeFunctionForLinkage(TR::Node *callNode, TR_DispatchType dispatchType,
    ...
    TR::Register *javaLitOffsetReg, TR::Register *methodAddressReg, bool isJNIGCPoint)
{
    ...
    javaReturnRegister = systemLinkage->callNativeFunction(callNode, deps, targetAddress, methodAddressReg,
    ...
    TR::RegisterDependencyConditions *&deps, bool isFastJNI, bool isPassReceiver, bool isJNIGCPoint)
{
    ...
    // todo: can be cleaned up better (i.e, get rid of some j9 only params) once other related linkage methods are
    // cleaned up
    self()->setupRegisterDepForLinkage(callNode, dispatchType, deps, killMask, systemLinkage, GlobalRegDeps,
        hasGlRegDeps, &methodAddressReg, javaLitOffsetReg);

    /*********************************/
    /* Build Arguments               */
    /*********************************/
    self()->setupBuildArgForLinkage(callNode, dispatchType, deps, isFastJNI, isPassReceiver, killMask, GlobalRegDeps,
    ...
    /*********************************/
    self()->performCallNativeFunctionForLinkage(callNode, dispatchType, javaReturnRegister, systemLinkage, deps,
        javaLitOffsetReg, methodAddressReg, isJNIGCPoint);
...
void OMR::Z::Linkage::lockRegister(TR::RealRegister *lpReal)
{
...
void OMR::Z::Linkage::unlockRegister(TR::RealRegister *lpReal)
{
    // literal pool register
 ...
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg, int32_t toreg)
{
 ...
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getLastSavedRegister(int32_t fromreg, int32_t toreg)
{
 ...
 * Get last used register between fromreg to toreg
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstRestoredRegister(int32_t fromreg, int32_t toreg)
{
 ...
 * Get last used register between fromreg to toreg
 */
TR::RealRegister::RegNum OMR::Z::Linkage::getLastRestoredRegister(int32_t fromreg, int32_t toreg)
{
...
TR::Instruction *OMR::Z::Linkage::restorePreservedRegs(TR::RealRegister::RegNum firstUsedReg,
    TR::RealRegister::RegNum lastUsedReg, int32_t blockNumber, TR::Instruction *cursor, TR::Node *nextNode,
    TR::RealRegister *spReg, TR::MemoryReference *rsa, TR::RealRegister::RegNum spRealReg)
{
    ...
    if (break64 || newNumUsedRegs == 1) {
        curReg = newFirstUsedReg;
        int32_t curDisp = rsa->getOffset();
        for (uint8_t i = 0; i < newNumUsedRegs; i++) {
            cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
                self()->getRealRegister(REGNUM(curReg)), rsa, cursor);
    ...
    else if ((self()->cg()->comp()->target().is64Bit() && !break64) || ((newFirstUsedReg + 1) % 2 == 0)) {
        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
            self()->getRealRegister(REGNUM(newFirstUsedReg)), self()->getRealRegister(REGNUM(newLastUsedReg)), rsa,
            cursor);
    } else // 32-bit (try to use bumps)
    {
        // Load Single + Load Multiple
        cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), nextNode,
            self()->getRealRegister(REGNUM(newFirstUsedReg)), rsa, cursor);

        rsa = generateS390MemoryReference(spReg, rsa->getOffset() + self()->machine()->getGPRSize(), self()->cg());
        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
            self()->getRealRegister(REGNUM(newFirstUsedReg + 1)), self()->getRealRegister(REGNUM(newLastUsedReg)), rsa,
            cursor);
    }
    return cursor;
}
...
int32_t OMR::Z::Linkage::getFirstMaskedBit(int16_t mask) { return TR::Linkage::getFirstMaskedBit(mask, 0, 15); }

int32_t OMR::Z::Linkage::getLastMaskedBit(int16_t mask) { return TR::Linkage::getLastMaskedBit(mask, 0, 15); }

bool OMR::Z::Linkage::isXPLinkLinkageType() { return self()->getLinkageType() == TR_SystemXPLink; }

bool OMR::Z::Linkage::isFastLinkLinkageType() { return self()->getLinkageType() == TR_SystemFastLink; }

bool OMR::Z::Linkage::isZLinuxLinkageType() { return self()->getLinkageType() == TR_SystemLinux; }

TR::Register *OMR::Z::Linkage::buildNativeDispatch(TR::Node *callNode, TR_DispatchType dispatchType)
{
    TR::RegisterDependencyConditions *deps = NULL;
    return self()->buildNativeDispatch(callNode, dispatchType, deps, false, true, true);
}

int32_t OMR::Z::Linkage::getOutgoingParameterBlockSize() { return self()->getLargestOutgoingArgumentAreaSize(); }

int32_t OMR::Z::Linkage::numArgumentRegisters(TR_RegisterKinds kind)
{
    switch (kind) {
        case TR_GPR:
...
void OMR::Z::Linkage::setIntegerArgumentRegister(uint32_t index, TR::RealRegister::RegNum r)
{
...
TR::RealRegister::RegNum OMR::Z::Linkage::getLongHighArgumentRegister(uint32_t index)
{
...
void OMR::Z::Linkage::setFloatArgumentRegister(uint32_t index, TR::RealRegister::RegNum r)
{
...
void OMR::Z::Linkage::setVectorArgumentRegister(uint32_t index, TR::RealRegister::RegNum r)
{
    _vectorArgRegisters[index] = r;
    self()->setRegisterFlag(r, VectorArgument);
}

TR::RealRegister::RegNum OMR::Z::Linkage::setIntegerReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, IntegerReturn);
    return _integerReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongLowReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, IntegerReturn);
    return _longLowReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongHighReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, IntegerReturn);
    return _longHighReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, IntegerReturn);
    return _longReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setFloatReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _floatReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setDoubleReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _doubleReturnRegister = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister0(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _longDoubleReturnRegister0 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister2(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _longDoubleReturnRegister2 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister4(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _longDoubleReturnRegister4 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setLongDoubleReturnRegister6(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, FloatReturn);
    return _longDoubleReturnRegister6 = r;
}

TR::RealRegister::RegNum OMR::Z::Linkage::setVectorReturnRegister(TR::RealRegister::RegNum r)
{
    self()->setRegisterFlag(r, VectorReturn);
    return _vectorReturnRegister = r;
}

TR::RealRegister *OMR::Z::Linkage::getStackPointerRealRegister()
{
    return self()->getRealRegister(_stackPointerRegister);
}

TR::RealRegister *OMR::Z::Linkage::getStackPointerRealRegister(uint32_t num)
{
    return self()->getRealRegister(_stackPointerRegister);
}

TR::RealRegister::RegNum OMR::Z::Linkage::getNormalStackPointerRegister() { return self()->getStackPointerRegister(); }

TR::RealRegister *OMR::Z::Linkage::getNormalStackPointerRealRegister() { return self()->getStackPointerRealRegister(); }

TR::RealRegister *OMR::Z::Linkage::getEntryPointRealRegister() { return self()->getRealRegister(_entryPointRegister); }

TR::RealRegister *OMR::Z::Linkage::getLitPoolRealRegister() { return self()->getRealRegister(_litPoolRegister); }

TR::RealRegister *OMR::Z::Linkage::getStaticBaseRealRegister() { return self()->getRealRegister(_staticBaseRegister); }

TR::RealRegister *OMR::Z::Linkage::getPrivateStaticBaseRealRegister()
{
    return self()->getRealRegister(_privateStaticBaseRegister);
}

TR::RealRegister *OMR::Z::Linkage::getReturnAddressRealRegister()
{
    return self()->getRealRegister(_returnAddrRegister);
}

TR::RealRegister *OMR::Z::Linkage::getVTableIndexArgumentRegisterRealRegister()
{
    return self()->getRealRegister(_vtableIndexArgumentRegister);
}

TR::RealRegister *OMR::Z::Linkage::getJ9MethodArgumentRegisterRealRegister()
{
    return self()->getRealRegister(_j9methodArgumentRegister);
}

TR::RealRegister *OMR::Z::Linkage::getRealRegister(TR::RealRegister::RegNum rNum)
{
