    ...
    while ((current != NULL) && !current->isLabel() && !current->isCall() && !current->isBranchOp()
        ...
        if (current->isLoad()) {
            ...
            if (disp == stackOffset) {
                ...
                //
                if (newInst && performTransformation(comp, "O^O : Prolog peeking removing [%p]\n", current)) {
    ...
    int64_t killMask, TR::Register *&vftReg, bool passReceiver, bool isRightToLeft)
{
    ...
    bool rightToLeft = self()->isParmsInReverseOrder() && !callNode->getSymbolReference()->isOSRInductionHelper() && isRightToLeft;
